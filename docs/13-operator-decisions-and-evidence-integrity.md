...
| Guard scanning an evacuated surface | protected code moves — a refactor relocates methods into a new package — while the guard's scan roots still point at the old location, so a source-scanning guard goes green by scanning nothing; an empty surface is indistinguishable from a clean tree | known-positive sentinel (one target the scan must always find — zero sentinels found means the guard is broken, not the tree clean); denominator floor that fails when the count of scanned targets drops below a pinned value; scan roots migrate in the same batch as any move of the protected code, verified by the sentinel re-firing at the new location (principle 31) |
...
| Inherited-debt baseline drifts upward invisibly | nothing diffs the debt inventory, so new type errors, skips, or known failures ride in under the old amnesty and the "budget" grows without anyone deciding it | pin each debt class as a checked-in fingerprint (sorted failure list, per-module count); acceptance is byte/count-identical against the pin and the ratchet is only-down — a decrease re-pins in the same change, an increase fails the lane (`templates/RESIDUAL_LEDGER.template.md` § Inherited-Debt Baselines) |
| Gating lane mass-skips its absent subject | a curated contract lane depends on an external runtime (database container, local daemon); the day the runtime is down, every dependent test skips and the lane reports green — the acceptance run is invalidated only if a human happens to read the skip count | fixtures honor `REQUIRE_<DEP>=1` flags that convert skip into hard failure, set unconditionally in the gating lane's command; the lane asserts zero skips or a pinned collected count; prove the conversion by mutation — against an unreachable dependency the flagged run must fail where the unflagged run skipped (principle 40) |
| Scope path carries the repository-directory prefix | the review runner emits an empty `reference_only_worktree` scope and the review executes while binding nothing (P0) | scope paths are project-relative without the repo prefix; verify the emitted scope mode is `pinned_commit_diff` before spending the review (`17-adversarial-design-review-loops.md` § 3) |
| Dormant gating lane cited as fresh evidence | the lane's CI trigger binds to pushes and the working branch is never pushed (or the trigger died); the lane's last run predates hundreds of local commits while signoff text keeps saying "CI green", and nothing in the claim's wording distinguishes it from a fresh verdict | every green claim names the run's date and head commit and is diffed against the claim's commits — a run older than the change fails closed as evidence; local executions of the lane command are recorded as the actual verification; the ledger preflight rejects rows citing dormant runs (`03-testing-strategy.md` § A Dormant Lane Is Not Evidence; principle 43's limit case) |
| Quota wall answered with a silent reviewer downgrade | a different model's verdict is not the pinned reviewer's verdict — the gate's evidence class is quietly invalidated | operator-selected channel fallback: byte-identical reviewer policy over a probed OpenAI-compatible relay; the channel is recorded in the checked-in reviewer policy and in every artifact's metadata, and evidence verification (effective tier, no reroute, causal binding) runs unchanged (`17-adversarial-design-review-loops.md` § 3; principle 35) |
